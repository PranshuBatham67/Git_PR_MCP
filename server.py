...
# Events file for storing GitHub webhook events
...
# Grabs all the changes done by developers
@mcp.tool()
async def analyze_file_changes(
    ...
    try:
        ...
        stat_result = subprocess.run(
            ["git", "diff", "--stat", f"{base_branch}...HEAD"], # Shows a list of files that have changed between base_branch and HEAD.
            capture_output=True,
            text=True,
            cwd=cwd
        )
        ...
        if include_diff:
            ...
            # Check if we need to truncate means check if the max_diff_lines <= 500
...
# Shows Claude the 7 templates to choose from
@mcp.tool()
...
# Claude picks “Feature” (smart choice!)
@mcp.tool()
...
# ===== Cloud Integration: Hugging Face AI Tools =====

@mcp.tool()
async def summarize_pr_changes(diff_content: str, max_length: int = 200) -> str:
    """Use Hugging Face AI to summarize pull request changes.
    
    Args:
        diff_content: The git diff content to summarize
        max_length: Maximum length of the summary in characters (default: 200)
    """
    api_token = os.getenv("HUGGINGFACE_API_TOKEN")
    if not api_token:
        return json.dumps({"error": "HUGGINGFACE_API_TOKEN environment variable not set"})
...
{diff_content[:2000]}  # Limit input to avoid token limits

Summary (keep under {max_length} characters):"""

        response = client.text_generation(
            prompt,
            model="microsoft/DialoGPT-medium",  # You can change to a summarization model like "facebook/bart-large-cnn"
            max_new_tokens=100,
            temperature=0.7
        )
...
@mcp.tool()
async def generate_pr_description(changes_summary: str, change_type: str) -> str:
    """Generate a detailed PR description using AI based on changes.
    
    Args:
        changes_summary: Summary of the changes made
        change_type: Type of change (feature, bug, refactor, etc.)
    """
    api_token = os.getenv("HUGGINGFACE_API_TOKEN")
    if not api_token:
        return json.dumps({"error": "HUGGINGFACE_API_TOKEN environment variable not set"})
    ...
    try:
        ...
        prompt = f"""Generate a professional pull request description for a {change_type} change.

Changes Summary: {changes_summary}

Please write a PR description that includes:
- What was changed
- Why it was changed
...
Keep it concise but informative."""
        ...
        return json.dumps({"error": f"Failed to generate PR description: {str(e)}"})
